...
app.set('url full', (options.url.ssl? 'https://': 'http://') + app.get('url host') + app.get('url prefix'));
app.set('basepath', app.get('url prefix'));
app.set('is_production', process.env.NODE_ENV === app.PRODUCTION);

if (options.url.static) {
  app.set('static url', (options.url.ssl? 'https://': 'http://') + app.get('url static'));
} else {
...
app.connect = function (callback) {
  ...
  app.use(filteredCookieSession(['passport', 'user.email', 'user.github_id', 'user.github_token', 'user.dropbox_token', 'user.dropbox_id', 'user.flagged', 'user.bincount', 'user.api_key', 'user.key', 'user.id', 'user.embed']));
