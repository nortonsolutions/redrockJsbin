...
module.exports = Observable.extend({
  constructor: function SessionHandler(sandbox) {
    ...
    var methods = Object.getOwnPropertyNames(SessionHandler.prototype).filter(
      function(prop) {
  ...
  deleteAccount: function(req, res, next) {
    ...
    if (
      req.body.confirm.trim().toLowerCase() !==
      req.session.user.name.trim().toLowerCase()
    ) {
      res.flash(
        req.flash.ERROR,
        "The confirmation username did not match your username, no dice I'm afraid."
      );
  ...
  setProAccount: function(req, res) {
    var setPro = true;
    this.models.user.setProAccount(
  ...
  updateUser: function(req, res, next) {
    ...
    if ('name' in req.params && req.user.name !== req.param('name')) {
      return this.respond(req, res, 400, {
        ok: false,
        message:
          "Sorry, changing your username isn't supported just yet. We're on it though!",
        step: '6',
      });
    }
    ...
    var updateOwnershipData = denodeify(
      session.models.user.updateOwnershipData
    ).bind(session.models.user);
    var updateKey = denodeify(session.models.user.updateKey).bind(
      session.models.user
    );
  ...
  forgotPassword: function(req, res, next) {
    ...
    if (!usernameOrEmailAddress) {
      ...
      res.json({
        error:
          'Either a username or email address is required to send a password reset token.',
      });
      return;
    }
        ...
        sessionHandler.mailer.forgotPassword(user.email, ctx, function(err) {
          ...
          if (err && err instanceof errors.MailerError) {
            ...
            if (sessionHandler.mailer.isEnabled()) {
              flash =
                'Sorry, an error occurred when sending the reset email: ' + err;
            } else {
              flash =
                'Unable to send reset, email is not enabled in this version of JS Bin.';
            }
          } else if (err) {
            return next(err);
          }
    ...
    var noUserFound = function() {
      ...
      res.json({
        error:
          "I looked, but I couldn't find any user with those details. Sorry!",
      });
    };

    // try loading by username first
    this.models.user.load(
      ...
      function(err, user) {
        ...
        if (!user) {
          // try by email next
          this.models.user.loadByEmail(usernameOrEmailAddress, function(
            err,
            user
          ) {
            if (!user) {
  ...
  resetPassword: function(req, res) {
    ...
    if (!token) {
      ...
      res.flash(
        req.flash.ERROR,
        'You need to provide a magic token to access your account'
      );
    ...
    this.models.forgotToken.loadUser(token, function(err, user) {
      if (err) {
        res.flash(
          req.flash.ERROR,
          'This token has failed, please use the reset token within 24 hours of creation'
        );
      ...
      if (!user) {
        res.flash(
          req.flash.ERROR,
          'This token has expired, please use the reset token within 24 hours of creation'
        );
        return res.redirect(303, _this.helpers.url());
      }
      ...
      res.flash(
        ...
        'You\'re now logged in, please make sure you update your password from your <a href="/account/profile">profile</a>'
      );
  ...
  setUserSession: function(req, user) {
    ...
    req.session.user.settings.reloaded = true;
  ...
  dropboxCallback: function(req, res, next) {
    ...
    if (req.session.user) {
      ...
      this.models.user.updateOwnershipData(
        req.session.user.name,
        {
        ...
        function(err) {
          ...
          res.flash(
            req.flash.NOTIFICATION,
            'Your dropbox account has been linked to JS Bin, all future bins will now be saved to dropbox'
          );
  ...
  githubCallback: function(req, res) {
    ...
    // Find user from GitHub id
    this.models.user.getOne(
      'getUserByGithubId',
      [githubUser.profile.id],
      function(err, jsbinUser) {
        if (err) {
          return this.respondWithMessage(
            res,
            res.flash.ERROR,
            'Sorry, there was an error while accessing the database with your GitHub id (' +
              err +
              ')'
          );
        }
        ...
        if (jsbinUser) {
          ...
          return this.models.user.updateGithubData(
            jsbinUser.name,
            jsbinUser.github_id,
            jsbinUser.github_token,
            function(err) {
              // jshint ignore:line
              if (err) {
                return this.respondWithMessage(
                  res,
                  res.flash.ERROR,
                  'Sorry, could not update your GitHub data & sign you in (' +
                    err +
                    ')'
                );
              }
        ...
        if (sessionUser) {
          ...
          return this.models.user.updateGithubData(
            sessionUser.name,
            sessionUser.github_id,
            sessionUser.github_token,
            function(err) {
              // jshint ignore:line
              if (err) {
                return this.respondWithMessage(
                  res,
                  res.flash.ERROR,
                  'Sorry, could not connect your GitHub and JS Bin accounts (' +
                    err +
                    ')'
                );
              }
              ...
              res.flash(
                res.flash.NOTIFICATION,
                'Your JS Bin account (' +
                  sessionUser.name +
                  ') has been linked to your GitHub account (' +
        ...
        this.models.user.load(
          githubUser.profile.username,
          function(err, jsbinUserFromGithubName) {
            if (err) {
              return this.respondWithMessage(
                res,
                res.flash.ERROR,
                'Sorry, there was an error while accessing the database with your GitHub username (' +
                  err +
                  ')'
              );
            }
            ...
            if (jsbinUserFromGithubName) {
              ...
              res.flash(
                res.flash.ERROR,
                'Your GitHub username is taken on JS Bin. If this is you, please register with JS Bin and link your GitHub account. <a target="_blank" href="http://jsbin.com/help/github-username">Read this for more help</a>.'
              );
              return res.redirect(redirect);
            }
                  ...
                  function(err) {
                    if (err) {
                      return this.respondWithMessage(
                        res,
                        res.flash.ERROR,
                        'Sorry, there was an error while creating a new user (' +
                          err +
                          ')'
                      );
                    }
                    ...
                    res.flash(
                      res.flash.NOTIFICATION,
                      'Welcome to JS Bin, ' + userData.name + '.'
                    );
  ...
  githubCallbackWithError: function(err, req, res, next) {
    ...
    if (err.oauthError && err.oauthError.statusCode === 401) {
      return this.respondWithMessage(
        res,
        res.flash.ERROR,
        'Sorry, looks like that auth code has expired, please try and login again.'
      );
    }
    next(err);
  },
