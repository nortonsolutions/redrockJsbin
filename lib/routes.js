...
function mountRouter(expressApp) {
  ...
  var sandbox = {
    ...
    helpers: helpers.createHelpers(expressApp),
  };
  ...
  function redirectToOutput(req, res, next) {
    ...
    if (output && req.headers.host.indexOf(config.url.host) === 0) {
      return res.redirect(
        (req.secure ? 'https://' : 'http://') + output + req.url
      );
    }
  ...
  function shouldNotBeSecure(req, res, next) {
    ...
    if (req.shouldNotBeSecure) {
      return res.redirect(
        'http://' + req.headers.host.replace(/:.*/, '') + req.url
      );
    }
  ...
  function featureByBinOwner(feature, handler) {
    return function(req, res, next) {
      if (
        features(feature, { session: { user: undefsafe(req, 'bin.metadata') } })
      ) {
        return next();
      }
  ...
  function redirectOffPreview(req, res, next) {
    ...
    if (output && req.headers.host === output) {
      return res.redirect(
        (req.secure ? 'https://' : 'http://') + config.url.host + req.url
      );
    }
  ...
  app.get(
    ['/issues', '/bugs'],
    redirect('https://github.com/jsbin/jsbin/issues/')
  );
  app.get(
    ['/video', '/videos', '/tutorials'],
    redirect(
      'http://www.youtube.com/playlist?list=PLXmT1r4krsTooRDWOrIu23P3SEZ3luIUq'
    )
  );
  ...
  app.get(
    ...
    denyframes,
    time('request.root'),
    userHandler.loadVanityURL,
    binHandler.loadBin,
    secureOutput,
    binHandler.getBinPreview
  );
  ...
  app.post('/processor', features.route('processors'), function(req, res) {
    processors
      .run(req.body.language, req.body)
      .then(function(data) {
        res.send(data);
      })
  ...
  app.get('/api/:bin/:rev?', binHandler.loadBin, function(req, res, next) {
    ...
    res.send({
      ...
      last_updated:
        undefsafe(req, 'bin.metadata.last_updated') || new Date().toJSON(),
      url: req.bin.url,
      snapshot: req.bin.revision,
    });
  });
  app.delete('/api/:bin/:rev?', binHandler.loadBin, binHandler.delete, function(
    req,
    res
  ) {
    res.send({
  ...
  app.post('/api/save', binHandler.createBin, binHandler.apiTrackBin, function(
    req,
    res,
    next
  ) {
  ...
  app.post(
    ...
    binHandler.createRevision,
    binHandler.apiTrackBin,
    function(req, res, next) {
      res.send({
        url: req.bin.url,
        snapshot: req.bin.revision,
  ...
  app.post('/account/new-api-key', function(req, res, next) {
    sandbox.models.user.generateAPIKey(req.session.user.name, function(
      error,
      key
    ) {
      if (error) {
  ...
  app.get('/account/assets/sign', features.route('!assets'), function(
    req,
    res,
    next
  ) {
    ...
    res.send(
      '<a href="/upgrade"><span class="pro-required">PRO</span></a> <a href="/upgrade">Asset uploading is a pro feature &ndash; upgrade today!</a>'
    );
  });

  app.get(
    '/account/assets/size',
    sessionHandler.requiresLoggedIn,
    features.route('assets'),
    assetHandler.size
  );
  app.post(
    '/account/assets/remove',
    sessionHandler.requiresLoggedIn,
    features.route('assets'),
    assetHandler.remove
  );
  ...
  app.get('/account/upgrade/pay', function(req, res, next) {
    if (!req.session.user) {
      ...
      req.flash(
        ...
        'Before upgrading to <strong>Pro</strong> you will need to create a free account or log in.'
      );
  ...
  app.get(
    ...
    features.route('pro'),
    upgradeHandler.subscription
  );
  app.post(
    '/account/subscription/cancel',
    features.route('pro'),
    upgradeHandler.cancel,
    redirect('/')
  );
  app.post(
    '/account/subscription/update-card',
    features.route('pro'),
    upgradeHandler.updateCard
  );

  app.get(
    '/upgrade',
    features.route('!upgradeWithFeatures'),
    upgradeHandler.features
  );
  app.get(
    '/upgrade',
    features.route('upgradeWithFeatures'),
    captureRefererForLogin,
    upgradeHandler.payment
  );
  ...
  var renderAccountSettings = (function() {
    ...
    return function renderAccountSettings(req, res) {
      ...
      var page =
        pages.indexOf(req.params.page) === -1 ? false : req.params.page;
  ...
  app.post(
    ...
    function(req, res) {
      ...
      sandbox.models.user.updateOwnershipData(
  ...
  app.post('/account/editor', features.route('accountPages'), function(
    req,
    res
  ) {
    if (!req.session || !req.session.user) {
    ...
    sandbox.models.user.updateSettings(
      req.session.user.name,
      settings,
      function(err) {
        if (err) {
  ...
  app.get(
    '/account/bookmark/vanity',
    features.route('vanity'),
    binParamFromReferer,
    binHandler.loadBin,
    userHandler.saveVanityURL
  );
  app.post(
    '/account/bookmark/vanity',
    features.route('vanity'),
    function(req, res, next) {
      ...
      res.send(
        400,
        'You need to be on a bin to publish it as the vanity home page'
      );
    },
    binHandler.loadBin,
    userHandler.saveVanityURL
  ...
  app.get('/account/bookmark/vanity', features.route('vanity'), function(
    req,
    res
  ) {
    res.send({});
  ...
  function renderLoginRegister(req, res) {
    ...
    if (req.query.firsttime) {
      res.flash(
        req.flash.NOTIFICATION,
        'We\'ve <a target="_blank" href="/blog/ssl"><strong>upgraded our login process to use SSL</strong></a>, however, this does mean  you have been logged out today, so please could you log in again below.<br><br><a href="http://github.com/jsbin/jsbin/issues/new" target="_blank">Questions/problems?</a>'
      );
    }
  ...
  app.post(
    '/login',
    sessionHandler.checkUserLoggedIn,
    userHandler.validateLogin,
    sessionHandler.loginUser,
    sessionHandler.redirectUserAfterLogin
  );

  app.post('/account/update', sessionHandler.routeSetHome);
  app.post(
    '/account/delete',
    sessionHandler.deleteAccount,
    function(req, res, next) {
    ...
    request(
      ...
      function(error, response, body) {
        ...
        var imgs = pixels.map(function(pixel) {
          return (
            '<img src="' +
            pixel.replace('[timestamp]', time) +
            '" height=1 width=1 border=0 style="display:none">'
          );
        });
  ...
  app.post(
    '/admin/user-verified',
    features.route('admin'),
    adminHandler.userVerified
  );
  ...
  app.get(
    ...
    sessionHandler.githubPassportCallback,
    sessionHandler.githubCallback
  );

  // DropBox auth
  app.get(
    '/auth/dropbox',
    features.route('dropbox'),
    sessionHandler.dropboxAuth
  );
  app.get(
    '/auth/dropbox/callback',
    sessionHandler.dropboxPassportCallback,
    sessionHandler.dropboxCallback
  );

  // List (note that the :user param is handled inside the getUserBins)
  app.get(
    '/list/:user',
    time('request.list.specific'),
    middleware.cors(),
    binHandler.getUserBins
  );
  app.get('/list', time('request.list'), binHandler.getUserBins);
  app.get(
    '/show/:user',
    time('request.homepage'),
    middleware.cors(),
    binHandler.getUserBins
  );
  app.get(
  ...
  app.get(
    '/:username/last(-:n)?/edit',
    secureOutput,
    binHandler.getLatestForUser,
    binHandler.getBin
  );
  app.get(
    '/:username/last(-:n)?/watch',
    binHandler.getLatestForUser,
    binHandler.live,
    binHandler.getBin
  );
  ...
  app.post(
    '/:bin/:rev?/delete',
    time('request.bin.delete'),
    features.route('delete'),
    binHandler.delete,
    function(req, res) {
      res.send(200, true);
    }
  ...
  app.post(
    '/:bin/:rev?/delete-all',
    time('request.bin.delete-all'),
    features.route('delete'),
    binHandler.deleteAll
  );
  ...
  app.get('/bin/user.js', nocache, function(req, res, next) {
    ...
    var user = _.pick.apply(
      _,
      [req.session.user || {}].concat(userfields.split(' '))
    );
    ...
    if (!user.avatar && req.session.user) {
      req.session.user.avatar = user.avatar = req.app.locals.gravatar(
        req.session.user
      );
    }
    ...
    res.render('user', {
      version: version,
      root: root,
      shareRoot: features('vanity', req)
        ? http + '://' + user.name + '.' + req.app.get('url host')
        : root,
      runner: runner,
      static: statik,
  ...
  app.get(
    ...
    redirectToOutput,
    time('request.source'),
    binHandler.getBinSource
  );

  app.get(
    '/:bin/:rev?.:format(' + Object.keys(processors.mime).join('|') + ')',
    redirectToOutput,
    sameoriginframes,
    time('request.source'),
    binHandler.getBinSourceFile
  );
  app.get(
    '/:bin/:rev?/:format(js)',
    redirectToOutput,
  ...
  app.get(
    '/:bin/:quiet(quiet)?',
    redirectToOutput,
    featureByBinOwner('pro', sameoriginframes),
    binHandler.testPreviewAllowed,
    binHandler.getBinPreview
  ); // spike.getStream,
  app.get(
    '/:bin/:rev?/:quiet(quiet)?',
