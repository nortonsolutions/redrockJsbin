...
module.exports = {
  ...
  titleForBin: function (bin) {
    ...
    if (matches.length === 2 && matches[1].trim()) {
      return truncateTitle(matches[1]);
    }
    ...
    if (matches) {
      return truncateTitle(matches);
    }

    // No title return JavaScript.
    if (javascript) {
      return truncateTitle(javascript);
    }

    if (css) {
      return truncateTitle(css);
    }
    ...
    return truncateTitle(html.replace(this.re.tags, '').trim());
  },
  since: function (date) {
